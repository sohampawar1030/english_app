...
const C = { title: 'Corporate Sentences', color: '#2563eb', desc: 'Office communication - meetings, emails, presentations, negotiations, deadlines' }

export default function CorporateSentencesPage() {
  ...
  function fetchSaved() {
    fetch('/api/sentences?category=corporate&page=' + page + '&limit=10')
  ...
  async function addSentence(sentence, translation) {
    ...
    try {
      const res = await fetch('/api/sentences', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ category: 'corporate', sentence, translation })
      })
      if (res.ok) {
  ...
  async function addManual(e) {
    ...
    try {
      const res = await fetch('/api/sentences', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ category: 'corporate', sentence: manual.trim(), translation: manualMr || null })
      })
      if (res.ok) {
  ...
  const SENTC = { color: '#1e40af' }
  const TRNSC = { color: '#2563eb', fontSize: '12px', marginTop: '2px' }
  ...
  return (
    <div style={{ padding: '24px', maxWidth: '960px', margin: '0 auto' }}>
      ...
      <div style={{ background: '#eff6ff', border: '1px solid #93c5fd', borderRadius: '10px', padding: '16px', marginBottom: '16px' }}>
        <div onClick={() => setFavOpen(o => !o)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: favOpen ? '12px' : 0, cursor: 'pointer', userSelect: 'none' }}>
          <strong style={{ fontSize: '15px', color: '#1e40af' }}>
            {favOpen ? '\u25BE' : '\u25B8'} My Favourite Corporate Sentences {favCount > 0 ? ' (' + favCount + ' starred)' : ''}
          </strong>
          <span style={{ fontSize: '13px', color: '#1e40af' }}>{saved.length} saved</span>
        </div>
        {favOpen
          ? (saved.length === 0
            ? <p style={{ color: '#999', fontSize: '13px' }}>No favourite corporate sentences yet. Generate or add manually below, then click star</p>
            : <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                {saved.map(s => (
                  <div key={s.id} style={{
                    padding: '8px 12px', borderRadius: '6px', fontSize: '13px',
                    background: s.is_important ? '#dbeafe' : '#fff',
                    border: s.is_important ? '1px solid #93c5fd' : '1px solid #e5e7eb',
                    ...
                    <div onClick={() => setSavedTr(prev => { const n = new Set(prev); if (n.has(s.id)) n.delete(s.id); else n.add(s.id); return n })} style={{ flex: 1, cursor: 'pointer' }}>
                      ...
                      {savedTr.has(s.id) && s.translation ? <div style={TRNSC}>{s.translation}</div> : null}
                    </div>
      ...
      <form onSubmit={addManual} style={{ display: 'flex', gap: '8px', marginBottom: '12px', flexWrap: 'wrap', alignItems: 'center' }}>
        <input value={manual} onChange={handleManualWord} placeholder="Type a corporate sentence..." required
        ...
        <input value={manualMr} onChange={e => setManualMr(e.target.value)} placeholder="Auto-translates..."
          style={{ flex: '1', minWidth: '160px', padding: '10px 14px', border: '1px solid #ddd', borderRadius: '8px', fontSize: '14px', color: '#2563eb' }} />
      ...
      <div style={{ display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap', marginBottom: '16px' }}>
        ...
        <button onClick={generate} disabled={loading}
          ...
          {loading ? 'Generating 40 sentences...' : 'Generate 40 Corporate Sentences'}
        </button>
      </div>
      ...
      {generated.length > 0
        ? <div style={{ marginBottom: '24px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              {generated.map((s, i) => (
                <div key={i} style={{ padding: '8px 12px', background: '#eff6ff', borderRadius: '6px', fontSize: '13px', display: 'flex', justifyContent: 'space-between', alignItems: 'start', gap: '8px' }}>
                  <div onClick={() => setTranslated(prev => { const n = new Set(prev); if (n.has(i)) n.delete(i); else n.add(i); return n })} style={{ flex: 1, cursor: 'pointer' }}>
                    <div style={{ fontWeight: 600, color: SENTC.color }}>{i + 1}. {s.sentence}</div>
                    {translated.has(i) && s.translation ? <div style={TRNSC}>{s.translation}</div> : null}
                  </div>
                  <button onClick={() => addSentence(s.sentence, s.translation)} disabled={saving}
                    style={{ padding: '4px 12px', fontSize: '12px', borderRadius: '5px', border: 'none', background: '#1e40af', color: '#fff', cursor: 'pointer', whiteSpace: 'nowrap' }}>+</button>
                </div>
              ))}
            </div>
          </div>
